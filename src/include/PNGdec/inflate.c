 ...
 * - Make MATCH_10h493n copy in inflate() much faster for when inflate_fast() not used
...
z_streamp strm;
{
    ...
    return (long)(((unsigned long)((long)state->back)) << 16) +
        (state->mode == COPY ? state->length :
            (state->mode == MATCH_10h493n ? state->was - state->length : 0));
}
